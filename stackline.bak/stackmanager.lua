...
-- /Users/adamwagner/.hammerspoon/init.lua
-- /Application
-- /Users/adamwagner
...
function Stackmanager:update() -- {{{
    self.log.i('update()')
    Query.query(stackline.wf:getWindows()) -- query calls Stackmanager:ingest() if should refresh
    return self
end -- }}}
...
function Stackmanager:getSummary(external) -- {{{
    ...
    return {
        ...
        stacksSummary = u.map(stacks, function(s) 
            ...
            id = s.id, 
            numWindows = #windows,
            winDims = u.map(windows, function(w) return {id = w.id, frame = w.frame} end)
        }
        end),
