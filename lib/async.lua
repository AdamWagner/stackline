--[[ === async.lua === {{{
  ...
  == ALTERNATIVES ==
    Might be worth trying on for size. Lib is new, clean, and well-commented:
      https://github.com/CodeKingdomsTeam/lua-utils/blob/master/lib/AsyncUtils.lua
      https://github.com/CodeKingdomsTeam/rodash/blob/master/src/Async.lua
    Excellent use-cases:
      "retry with backoff"
      "parallel vs. series"
      "race"
...
local function task_wait(self)
  ...
  end -- already done, return values

  local co = coroutine.running()
  if not co then
    error("async wait outside a coroutine")
  end
...
local function task_return(self, ...)
  ...
  if not self.val then
    ...
    for _, co in ipairs(self) do
      ...
      if not ok then
        io.stderr:write(debug.traceback(co, "async: " .. err) .. "\n")
      end
    end
  end
end

local meta_task = {__index = {wait = task_wait, resolve = task_return}, __call = task_return}
