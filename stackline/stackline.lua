local defaultDeps = {
  __index = {
    ...
    config       = require 'stackline.stackline.configManager'(),
    manager      = require 'stackline.stackline.stackmanager',
    window       = require 'stackline.stackline.window',
    -- query        = require 'stackline.stackline.query',
  },
}

return function(deps)
  -- this module returns a function that, given explicit dependencies,
  -- will return the main stackline table
  deps = setmetatable(deps or {}, defaultDeps)

  local wf    = hs.window.filter
  local timer = hs.timer.delayed
  local click = hs.eventtap.event.types['leftMouseDown'] -- fyi, see hs.eventtap.event.types to see all event types
  -- hs.logger.historySize(10000) -- save last 10k stackline.log.msgs

  local stackline = {
    -- log    = hs.logger.new('stackline'),
  ...
  -- stackline.log.setLogLevel(0)
  stackline.log.i("Loading module...")

  function stackline:init(userConfig) -- {{{
    stackline.log.i('init()')
    ...
    self.config:init( -- default conf + user overrides
      table.merge(require 'stackline.conf', userConfig)
    )

    self.window = self.window({config = self.config, stackline = self})

    -- init stackmanager, and run update right away
    self.manager = self.manager {stackline = self}
    self.manager:init()
    self.manager:update()
    ...
    self.log.i('wf:subscribe() to full reload events')
    self.wf:subscribe(self.windowEvents, function(_win, _app, _evt) -- {{{
      self.queryWindowState:start()
    end) -- }}}

    -- On each win evt listed, simply *redraw* indicators
    -- No need for heavyweight query + refresh
    stackline.log.i('wf:subscribe() to redraw-only events')
    self.wf:subscribe({ -- {{{
      wf.windowFocused,
      wf.windowNotVisible,
      wf.windowUnfocused,
  ...
  function stackline:refreshClickTracker() -- {{{
    ...
    if turnedOn then -- only start if feature is enabled
      self.log.d('features.clickToFocus is enabled!')
      self.clickTracker:start()
    else
      self.log.d('features.clickToFocus is disabled')
  ...
  stackline.log.i('hs.spaces.watcher.new(…)')
