-- local log = hs.logger.new('window', 0)
-- log.i("Loading window")
...
return function(deps)
    ...
    function Window:new(hsWin) -- {{{
        ...
        -- u.pheader('hs win in Window:new')
        -- u.p(getmetatable(hsWin))
        local ws = {
            ...
            screen     = hsWin:screen():id(),             -- hs.screen object for window
    ...
    function Window:setupIndicator() -- {{{
        -- Config
        self.config = self.stackline.config:get('appearance')
        local c = self.config
    ...
    function Window:drawIndicator(overrideOpts) -- {{{
        ...
        -- self.indicator:mouseCallback(function(indicator, evt, id, x, y)
        --     print(evt)
        --     print(id)
        --     print('x,y:',x,y)
        -- end)
        -- self.indicator:canvasMouseEvents(false,false,false,false)
    ...
    function Window:getScreenSide() -- {{{
        ...
        -- NOTE: Alt implementation here: https://github.com/kev-zheng/dotfiles/blob/master/hammerspoon/.hammerspoon/spaces.lua#L40
    ...
    function Window:getIndicatorPosition()  -- {{{
        ...
        local xval
        local c = self.config
        self.screenFrame = self.screen:frame().table
            ...
            stack = {
                ...
                ['false'] = {
                    window = {
                        ...
                        ['false'] = {
                            bg = u.extend(u.copy(opts.color), {
                                alpha = self.stackline.manager:getShowIconsState() and 0 or 0.2,
                            }),
    ...
    function Window:getShadowAttrs() -- {{{
        ...
        local iconsDisabledDimmer = self.stackline.manager:getShowIconsState() and 1 or 5
    ...
    function Window:makeStackId(hsWin) -- {{{
        -- u.pheader('hsWin in make stack ID')
        -- u.p(hsWin)
        ...
        local fuzzFactor = self.stackline.config:get('features.fzyFrameDetect.fuzzFactor')
